...
test_expect_success 'egpg key recover (lost dongle)' '
    ...
    egpg key recover "$(pwd)"/$KEY_ID.key.* | grep "The key was recovered and imported successfully." &&
    test_must_fail ls "$(pwd)"/$KEY_ID.key.* &&
...
test_expect_success 'egpg key recover (backup file not found)' '
    split &&
    egpg key recover "$(pwd)"/$KEY_ID.key.x 2>&1 | grep "File not found"
'

test_expect_success 'egpg key recover (check backup file extension)' '
    split &&
    mv "$(pwd)"/$KEY_ID.key.* "$(pwd)"/$KEY_ID.key.xyz
    egpg key recover "$(pwd)"/$KEY_ID.key.xyz 2>&1 | grep "The name of the backup partial key must be like"
'
...
test_expect_success 'egpg key recover (key not split)' '
    split &&
    cp "$(pwd)"/$KEY_ID.key.* "$(pwd)"/key.bak &&
    egpg key join &&
    mv "$(pwd)"/key.bak "$(pwd)"/$KEY_ID.key.012 &&
    egpg key recover "$(pwd)"/$KEY_ID.key.012 2>&1 | grep "The key $KEY_ID is already available as a full key."
'
...
test_expect_success 'egpg key recover (missing partial key)' '
    ...
    rm "$EGPG_DIR"/.gnupg/$KEY_ID.key.* &&
    rm "$DONGLE"/.gnupg/$KEY_ID.key.* &&
    egpg key recover "$(pwd)"/$KEY_ID.key.* 2>&1 | grep "Could not find an other partial key for $KEY_ID"
'
...
test_expect_success 'egpg key recover (missing partial key)' '
    ...
    rm "$EGPG_DIR"/.gnupg/$KEY_ID.key.* &&
    rm -rf "$DONGLE"/.gnupg/ &&
    egpg key recover "$(pwd)"/$KEY_ID.key.* 2>&1 | grep "Directory not found: $DONGLE/.gnupg"
'
...
test_expect_success 'egpg key recover (missing dongle dir)' '
    ...
    rm "$EGPG_DIR"/.gnupg/$KEY_ID.key.* &&
    rm -rf "$DONGLE" &&
    egpg key recover "$(pwd)"/$KEY_ID.key.* 2>&1 | grep "The dongle directory not found: $DONGLE"
'

...
cmd_key_split() {
    ...
    get_gpg_key
    local key_id=$GPG_KEY
    if ls "$EGPG_DIR"/$GPG_KEY.key.* >/dev/null 2>&1; then
        fail "There is already a partial key for $GPG_KEY on $EGPG_DIR"
    fi
    ...
    local fingerprint=$(gpg --list-keys --with-colons --fingerprint $KEY_ID | grep '^fpr:' | cut -d: -f10)
